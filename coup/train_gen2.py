...
class Gen2Zoo:
    ...
    def __call__(self):
        r = random.random()
        if r < 0.5:
            # 50% chance to play the generalized Random Zoo to prevent catastrophic forgetting
            return ZooAgent.random_profile()
        elif r < 0.75:
            # 25% chance to practice specifically against the Heuristic
            return HeuristicAgent()
        else:
            # 25% chance to practice specifically against CFR
            if self.cfr_agent is None:
                self.cfr_agent = CFRAgent("cfr_model.json")
            return self.cfr_agent
...
def main():
    ...
    parser.add_argument("--base-model", type=str, default="ppo_model_gen1.pt")
    parser.add_argument("--output", type=str, default="ppo_model_gen2.pt")
    ...
    try:
        ...
        print(f"Loaded Gen 1 model {args.base_model} for fine-tuning!")
